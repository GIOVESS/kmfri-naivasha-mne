...
return size;
    ...
    `)),e.hasVVOpacity?(i.add("opacityFeatureAttribute","float"),t.constants.add("vvOpacityNumber","int",8),t.uniforms.add(new dt("vvOpacityValues",o=>o.vvOpacity.values,_t),new dt("vvOpacityOpacities",o=>o.vvOpacity.opacityValues,_t),new q("vvOpacityFallback",o=>o.vvOpacity.fallback,{supportsNaN:!0})),t.code.add(c`
...
return worldToScreenPerDistanceRatio / segmentDistanceToCamera;
  ...
  `),s.uniforms.add(new bi("stipplePatternTexture",n=>n.stippleTexture),new q("stipplePatternPixelSizeInv",n=>1/Rt(n))),e.stippleOffColorEnabled&&s.uniforms.add(new De("stippleOffColor",n=>xa(n.stippleOffColor))),a.include(qe),s.code.add(c`float getStippleSDF(out bool isClamped) {
...
vTransitionType = timeStamps.w;`);const{animation:o}=e;o===3&&n.constants.add("decayRate","float",2.3),n.code.add(c`
    float getTrailOpacity(float x) {
      ${La(o)}
    }`),n.uniforms.add(new q("timeElapsed",l=>l.timeElapsed),new q("trailLength",l=>l.trailLength),new q("speed",l=>l.animationSpeed),new De("timingOptions",l=>ve(Aa,l.startTime,l.endTime,l.fadeInTime,l.fadeOutTime))),n.code.add(c`float fadeIn(float x) {
...
return animatedColor;
}`)}function La(a){switch(a){case 2:return"return x >= 0.0 && x <= 1.0 ? 1.0 : 0.0;";case 3:return`float cutOff = exp(-decayRate);
        return (exp(-decayRate * x) - cutOff) / (1.0 - cutOff);`;default:return"return 1.0;"}}const Aa=Ze(),Se=1;function Wt(a){const e=new la,{attributes:t,varyings:i,vertex:s,fragment:n}=e,{applyMarkerOffset:o,draped:l,output:r,capType:d,stippleEnabled:p,falloffEnabled:v,roundJoins:g,wireframe:u,innerColorEnabled:f,hasAnimation:y,hasScreenSizePerspective:b}=a;n.include(yi),e.include(Sa,a),e.include($a,a),e.include(_i,a),e.include(xi,a),e.include(Da,a);const U=o&&!l;U&&(s.uniforms.add(new q("markerScale",m=>m.markerScale)),e.include(Ca,{space:2,hasScreenSizePerspective:b})),Ti(s,a),s.uniforms.add(new $i("inverseProjectionMatrix",m=>m.camera.inverseProjectionMatrix),new Pi("nearFar",m=>m.camera.nearFar),new q("miterLimit",m=>m.join!=="miter"?0:m.miterLimit),new wi("viewport",m=>m.camera.fullViewport)),s.constants.add("LARGE_HALF_FLOAT","float",65500),t.add("position","vec3"),t.add("previousDelta","vec4"),t.add("nextDelta","vec4"),t.add("lineParameters","vec2"),t.add("u0","float"),i.add("vColor","vec4"),i.add("vpos","vec3",{invariant:!0}),i.add("vLineDistance","float"),i.add("vLineWidth","float");const X=p;X&&i.add("vLineSizeInv","float");const h=d===2,L=p&&h,A=v||L;A&&i.add("vLineDistanceNorm","float"),h&&(i.add("vSegmentSDF","float"),i.add("vReverseSegmentSDF","float")),s.code.add(c`vec2 perpendicular(vec2 v) {
return vec2(v.y, -v.x);
}
...
discardByTerrainDepth();`),e.include(qe),n.main.add(c`
    ...
    ${j(A,c`float lineDistanceNorm = noPerspectiveRead(vLineDistanceNorm);`)}
  `),u?n.main.add(c`vec4 finalColor = vec4(1.0, 0.0, 1.0, 1.0);`):(h&&n.main.add(c`
      ...
      `):n.main.add(c`float stippleAlpha = getStippleAlpha(lineWidth);`),r!==10&&n.main.add(c`discardByStippleAlpha(stippleAlpha, ${c.float(ce)});`),e.include(qe),n.uniforms.add(new De("intrinsicColor",m=>m.color)).main.add(c`vec4 color = intrinsicColor * vColor;
color.a = noPerspectiveRead(color.a);`),f&&n.uniforms.add(new De("innerColor",m=>m.innerColor??m.color),new q("innerWidth",(m,$)=>m.innerWidth*$.camera.pixelRatio)).main.add(c`float distToInner = abs(lineDistance) - innerWidth;
